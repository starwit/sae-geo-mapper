...
# Changelog
## 1.1.0
- Add camera config option `ignore_edge_detections`: If set, detections whose bounding box reaches the image border (within a small tolerance) are not mapped (image space counterpart to the `mapping_area` geo-fence). Like geo-fenced detections, they are subject to `remove_unmapped_detections`.
## 1.0.0
